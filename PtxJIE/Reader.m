...
{
    NSData *dataNoLimiter = [NSData dataWithBytes:[data bytes] length:data.length -1];
    NSUInteger lineNumber = self.lineNumber;
    self.lineNumber = lineNumber + 1;
    if (0 < data.length) {
        NSString *line = [[NSString alloc] initWithData:dataNoLimiter encoding:NSUTF8StringEncoding];
