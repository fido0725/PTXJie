...
-(void)ComputeSolarTermForYear:(int)theYear intoList:(NSArray **)listValue
{
    NSMutableArray *marray = [NSMutableArray array];
    struct SolarTerm solarTermAllYear[24];
    for (int n = 1; n <25; n++)
    {
        double Termdays = [self Term:theYear:n:YES];
        double mdays = [self AntiDayDifference:theYear:floor(Termdays)];
        //double sm1 = floor(mdays / 100);
        int hour = (int)floor((double)[self Tail:Termdays] * 24);
        int minute = (int)floor((double)([self Tail:Termdays] * 24 - hour) * 60);
        int tMonth = (int)ceil((double)n / 2);
        ...
        if (n >= 3)
            solarTermAllYear[n-1].solarName = [SolarTerms objectAtIndex:(n - 3)];
        else
            solarTermAllYear[n-1].solarName = [SolarTerms objectAtIndex:(n + 21)];
        ...
        solarTermAllYear[n-1].solarDate = [[dateFormatter stringFromDate:ldate] intValue];
        NSMutableDictionary *mDict = [NSMutableDictionary dictionary];
        [mDict setValue:solarTermAllYear[n-1].solarName forKey:@"solarName"];
        [mDict setValue:[dateFormatter stringFromDate:ldate] forKey:@"solarDate"];
        [marray addObject:[mDict copy]];
        //[mDict release];
    }
    *listValue =[NSArray arrayWithArray:[marray copy]] ;
    //[marray release];
}
